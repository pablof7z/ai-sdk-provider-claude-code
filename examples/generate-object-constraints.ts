...
async function example1_enumsAndStatus() {
  ...
  const taskSchema = z.object({
    task: z.object({
      ...
      labels: z
        .array(z.enum(['frontend', 'backend', 'database', 'api', 'ui', 'performance', 'security']))
        .min(1)
        .max(3)
        .describe('1-3 relevant labels'),
    }),
  });
      ...
      socialMedia: z.object({
        ...
        linkedin: z
          ...
          .regex(
            ...
            'LinkedIn profile URL must start with https://linkedin.com/in/'
          )
      ...
      preferences: z.object({
        timezone: z
          ...
          .regex(
            ...
            'Timezone format like America/New_York'
          ),
  ...
  const playlistSchema = z.object({
    playlist: z.object({
      ...
      tracks: z
        ...
        .min(5)
        .max(20)
        .describe('Playlist must have 5-20 tracks'),
      tags: z
        .array(z.string().min(2).max(20))
        .min(3)
        .max(10)
        .describe('3-10 tags for the playlist'),
      collaborators: z.array(z.string().email()).max(5).describe('Up to 5 collaborator emails'),
      artwork: z.object({
        colors: z
